...
- [x] T0401 Storage Slice 12.1: Read-only Blockgeräte-Inventar (`GET /api/v1/storage/disks`), Eligibility-Klassifikation, `internal/platform/storageinventory`, Tests und `docs/05_STORAGE.md`.
- [x] T0402 Storage Stage 3 / Slice 11.1: Storage/NAS UI foundation — read-only **Storage inventory** in der lokalen WebGUI (`StorageShell`, `storageDto`, Navigation von Overview); `GET /api/v1/storage/disks`; ehrliche Grenze (keine Pools/Shares/Mutation); Frontend-Tests; Landing i18n aktualisiert (Storage-Inventar verfügbar, Pools deferred); `docs/05_STORAGE.md`, `docs/06_DASHBOARD.md`.
- [x] T0403 Storage Slice 12.2: Storage layout foundation — `GET /api/v1/storage/pools`, `GET /api/v1/storage/shares`, Overview-Feld `storage`, Storage/Overview UI-Layout-Panels; ehrliche leere Zustände (`mutation_available: false`); Gap-Audit `docs/ops/appliance-product-gap-audit-2026-07-16.md`; Final `docs/ops/appliance-product-gap-final-2026-07-16.md`.
- [ ] T0404 Storage: Pool-Mutation (Create/Expand) — Design + sichere Implementierung; abhängig von Slice 12.2 Inventar.
...
- [x] T0901 Updates Slice 9.10: Release-Manifest `vyntrio-release-manifest-v1` + lokale SHA-256-Verifikation via `vyntrio-verify-artifact`; Signaturfeld reserviert, nicht verifiziert (`docs/ops/release-artifact-verification.md`).
- [x] T0902 Updates Slice 10.10: Install-Target- und Media-Preflight via `vyntrio-installer preflight` (`--target-disk-id`, optionale Envelope/Release-Checks); `docs/ops/install-target-preflight.md`.
- [x] T0903 Updates Slice 10.11: Begrenzter Payload-Install-Write via `vyntrio-installer install` (`--force`, Preflight-Gates, Sandbox-only); `docs/ops/install-payload-write.md`.
- [x] T0904 Updates Slice 10.12: Postflight/Handover (`HANDOVER_RECORD.txt`, `vyntrio-installer postflight`); `docs/ops/install-staged-workflow.md`.
- [x] T0905 Updates Slice 10.13: Ownership-aware guarded target mutation via `vyntrio-installer install --apply-target` (mount single eligible partition, six payloads, rollback on failure); `docs/ops/install-target-mutation.md`.
- [x] T0906 Updates Slice 10.14: Existing-partition apply via `vyntrio-installer apply` (`installapply` package, `PARTITION_APPLY_RECORD.txt`, rollback tests); `docs/ops/install-partition-apply.md`.
- [x] T0907 Updates Slice 10.15: Installer-Policy und Flow-Konsolidierung (`installpolicy`, `vyntrio-installer workflow`, CLI-Hilfe, `docs/ops/installer-policy.md`).
- [x] T0908 Docs: USB-first-Produktausrichtung — ADR-/Policy-Realignment (`docs/00_PROJECT.md`, ADR-0006/0007, `docs/15_LICENSE.md`, sekundäre Block-10-Einordnung).
- [x] T0909 Updates Slice 9.11: Bootable-media foundation (`make install-media-bootability`, boot/live_root stubs, non-bootable image stub); `docs/ops/install-media-bootability.md`.
- [x] T0910 Updates Slice 9.12: Real boot chain + image emission (`make install-media-image`, real kernel/initrd/GRUB core + grub.cfg, live_root runtime additions, stub-vs-real validation, best-available image); firmware-bootable ISO/raw noch blockiert (kein xorriso/genisoimage, keine FAT-/Loop-Tools, kein EFI-GRUB-Target); `docs/ops/install-media-image.md`.
- [x] T0911 Updates Slice 9.13: Firmware-bootable Wrapper (`make install-media-wrap`) — reale MBR/BIOS-Raw-Image-Emission via `grub-bios-setup` (mke2fs/debugfs/sfdisk, ohne root/mount) plus strukturelle Verifikation (MBR 0x55AA, GRUB-Embed, bootfähige ext2-Partition); ISO-Strategie bei vorhandenem xorriso/genisoimage; ehrlicher Fallback mit konkreten Blockern (kein ISO-Writer, tmpfs-Build-Dir, kein UEFI/x86_64-efi, kein qemu für Runtime-Boot-Test); Drei-Achsen-Validierung; `docs/ops/install-media-wrapper.md`.
- [x] T0912 Updates Slice 9.14: Runtime-Boot-Verifikation + First-Boot-Dashboard-Pfad (`make install-media-runtime`) — gated qemu-Boot-Harness (headless Raw-Image-Boot, Serial-Log, GRUB/Kernel-Marker) mit ehrlichem Skip hier (kein qemu/kvm, `/dev/kvm` fehlt); First-Boot-Wiring in live_root (`firstboot.sh` startet vyntrio-api auf :8080, `vyntrio-firstboot.service`, live-init-Aufruf); `runtime_boot_tested`/`dashboard_reachable` ehrlich in `RUNTIME_BOOT.txt`; Dashboard erst nach Live-Userland + Boot-Umgebung erreichbar; `docs/ops/install-media-runtime-boot.md`.
- [x] T0913 Updates Slice 9.15: Minimale Live-Rootfs-Userland (`make install-media-live-rootfs`) — Loader + vollständige ldd-Closure, busybox (Shell/Coreutils), `vyntrio-api` als Live-Dashboard-Runtime, `/init` → `firstboot.sh`, Live-Initramfs (`vyntrio-live-initramfs.cpio.gz`, `/init` im Archiv-Root); verifiziert per chroot-Exec + Dashboard-HTTP-Probe (**HTTP 200**, Throwaway-Copy auf Port 18080, Shipped-live_root bleibt state-frei); **kein Boot** — Image-initrd noch nicht getauscht (keine Kernel-Module, kein VM zur Verifikation), `dashboard_reachable_on_boot: false`; live_root-Ausschlussgrenze präzisiert (Live-Runtime ≠ Target-Disk-Payload); `docs/ops/install-media-live-rootfs.md`.
- [x] T0914 Updates Slice 9.16: Live-Initramfs Hardware-Enablement (`make install-media-hardware`) — dep-aufgelöstes Storage/Netzwerk-Kernelmodul-Set (an den Boot-Chain-Host-Kernel gebunden) nach `live_root/lib/modules/<kver>/`, `modules.dep` via `depmod -b` regeneriert, geordnete `modules.load`, Modul-Load + **DHCP**-Bring-up (`hw-init.sh` + busybox `udhcpc`, `default.script`) in `/init` **vor** dem First-Boot-Pfad, Live-Initramfs neu emittiert (trägt jetzt Module); verifiziert per chroot-Exec + `modinfo` + Initramfs-Inhalt (`modules.dep` vorhanden); Module werden **nur gestaged, nicht geladen** (modprobe würde den Build-Host-Kernel treffen); `wired_into_image_initrd`/`boot_verified`/`dashboard_reachable_on_boot` bleiben ehrlich **false**; Live-Runtime ≠ Target-Disk-Payload gewahrt (keine `payload/lib/modules`); `docs/ops/install-media-hardware-enable.md`.
- [x] T0915 Updates Slice 9.17: Image-Initrd-Swap auf das Vyntrio Live-Initramfs (`make install-media-initrd-swap`) — ersetzt `boot/initrd.img` (Host-Debian-Initrd aus 9.12) durch das hardware-enabled Live-Initramfs (`build/vyntrio-live-initramfs.cpio.gz`) und baut das Firmware-Image über den bestehenden 9.13-Wrapper neu; **bewiesen** per sha256 des aus dem Image extrahierten Initrd (`debugfs dump` von der Partition) — stimmt mit dem Live-Initramfs überein und unterscheidet sich vom Host-Initrd; extrahiertes Initrd enthält `/init` + busybox + Kernelmodule; strukturelle Verifikation bleibt `pass`; **kein** Runtime-Boot (kein qemu) → `runtime_boot_tested`/`dashboard_reachable_on_boot` ehrlich **false**; fail-closed mit Host-Initrd-Restore bei Fehlschlag (keine Regression), idempotent; Live-Runtime ≠ Target-Disk-Payload gewahrt; `docs/ops/install-media-initrd-swap.md`.
- [x] T0916 Updates Stage 2 / Slice 10.1: Runtime-Boot-Verifikation auf echtem Boot-Harness (`make install-media-runtime-verify`) — **read-only** Verifier, der das initrd-getauschte Image (Stage 1) auf einem VM/Hardware-Harness bootet und einen **echten HTTP-200-Dashboard-Nachweis vom gebooteten Gast** (qemu user-net hostfwd) erbringt, oder **fail-closed** mit exakten Blockern skippt; ändert **niemals** Image/Initrd/Bootloader/live-rootfs (per sha256 vor/nach im Test erzwungen). Auf diesem Host: kein `qemu`/`/dev/kvm` → `status: blocked`, `runtime_boot_tested: false`, `dashboard_reachable_on_boot: false`, `chain_modified: false`, exakte Blocker aufgezeichnet. Dashboard-on-Boot-Nachweis = echte HTTP-Antwort, **nicht** chroot/strukturell. Erste Stage-2-Slice; unabhängig vom staged-installer Block 10. `docs/ops/install-media-runtime-verify.md`.
- [x] W11.1 Website-Linie / Landing-Foundation (Slice 11R.13): kundenorientierte Landing-Foundation vervollständigt — neue **Use-cases**-Sektion (vorgesehene Nutzer, keine ausgelieferten Features) als `PublicUseCaseSection` (DE/EN via `useCases`-i18n, Anker `#use-cases`) in die geteilte `PublicLandingView` (Produktion `/` + Preview) eingefügt; Foundation-Sektionsset jetzt vollständig: Hero, Value/Release, Pillars, **Use cases**, Control-Surface-Preview, Produktgrenze/Status, Primary+Final CTA, Footer/Nav. Original getextet, Unraude-IA nur high-level gespiegelt (kein kopierter Text/Branding/Layout). Ehrlich: keine Remote-Connect-/Lizenz-/Pricing-Aussagen; Produktstand Stage 2 (lokales Dashboard), keine öffentlichen Installationsmedien. Komponententest ergänzt; `docs/22_WEBSITE.md` (11R.13 + Website-Linie).
- [x] W11.2 Website Creator / media / self-hosting (Slice 11R.16): Landing um Creator-/Medien-Zielgruppe und ehrliche Lokalfähigkeiten erweitert — `#creators` (`creatorAudience`, `PublicUseCaseSection`, DE/EN) mit Medien-Bibliotheken, Projekt-Archiven, Self-hosting-Basis, Homelab/Studio; `#local-today` (`localToday`, `PublicProcedureOutline`) listet nur implementierte lokale Fähigkeiten und explizite Deferred-Grenzen (kein Remote Connect, keine Lizenz, kein öffentliches Install-Medium, keine NAS-Mutation). Originaltext, keine Unraid-Kopie. Tests: `LandingPage.test.tsx`, `public-components.test.tsx`. Deploy auf `https://vyntrio.xyz/` verifiziert. `docs/22_WEBSITE.md` 11R.16.
- [x] T0917 Updates Stage 2 / Slice 10.2: Lokale Dashboard-Stabilität nach Boot (dashboard-first WebGUI) — **echter Defekt behoben**: `firstboot.sh` lief unter `#!/usr/bin/env bash`, aber die Live-Initramfs liefert nur busybox (kein bash/`env`), d.h. der Dashboard-Entrypoint konnte im Live-Runtime **nie** starten; jetzt `#!/bin/sh` (busybox/POSIX). Dashboard wird **beaufsichtigt** (begrenzter Respawn-Loop, `VYNTRIO_DASHBOARD_MAX_RESTARTS`=5) → die WebGUI bleibt als Appliance-Dienst oben, statt still zu verschwinden; klare URL-Ausgabe für den Operator. Lokale **Readiness** zusätzlich bewiesen: der Wegwerf-Chroot-Probe (compose-live-rootfs) prüft nun `/readyz` (DB-ready), nicht nur `/` — hier `readyz=200`. Ehrlich: LAN-Exposure ist **auf TLS blockiert** (`cookie_secure`-Invariante verlangt für Nicht-Loopback-Bind TLS) → `lan_exposure_blocked_on: tls_required_for_non_loopback_bind`; `dashboard_reachable` bleibt `false` (kein qemu-Boot-Nachweis). Kein Installer/Target-Write, Boot/Image-Chain unverändert (nur First-Boot-Wiring). Tests erweitert (busybox-sh-Shebang, Supervise-Loop, readyz-Honesty); `docs/ops/install-media-dashboard-stability.md`.
- [x] T0918 Updates Stage 2 / Slice 10.3: First-Boot-Lokale-Onboarding-Klarheit + passende Landing-Seiten-Erweiterung. Produktseite: `firstboot.sh` gibt eine klare First-Boot-Setup-Banner-Ausgabe (Boot → lokales Dashboard auf <host>:8080 → Anmeldung/Owner-Erstellung → read-only Übersicht; Storage/Services/Remote später), in `FIRST_BOOT.txt`/`RUNTIME_BOOT.txt` als `first_boot_onboarding_clarity: clear` + `local_onboarding_steps` aufgezeichnet. Website: Neue Landing-Sektion "First boot / local setup" (`#first-boot-setup`, DE/EN) mit denselben 4 lokalen Schritten, `PublicProcedureOutline`, kein API, ehrlich (kein LAN-wide, kein Remote, keine Lizenz). Direkt an die Produkt-Wahrheit gekoppelt; `docs/22_WEBSITE.md` 11R.14 + Website-Linie.
- [x] T0919 Updates Stage 2 / Slice 10.4: TLS-Readiness / LAN-Dashboard-Bind-Vorbereitung — **Config-Blocker entfernt**: `vyntrio-api` unterstützt optionale `tls_cert_file`/`tls_key_file`, `ListenAndServeTLS` bei gesetztem Paar; Nicht-Loopback-Bind weiterhin nur mit `cookie_secure=true` + TLS (`internal/platform/config/tls.go`). Runtime: `prepare-live-dashboard-tls.sh` (busybox-sh) erzeugt ephemere Self-Signed-Certs unter `/run/vyntrio` wenn `VYNTRIO_LAN_BIND_IP` gesetzt; `firstboot.sh` nutzt Runtime-Overlay; Loopback-HTTP bleibt Default. Build: HTTPS-Wegwerf-Chroot-Probe (`/readyz` over TLS), `tls_readiness: ready` in `FIRST_BOOT.txt`/`RUNTIME_BOOT.txt`/`LIVE_ROOTFS.txt`; alter `lan_exposure_blocked_on: tls_required_for_non_loopback_bind` entfernt. Tests: `tls_readiness_test.sh`, Go-Unit-Tests. `docs/ops/install-media-dashboard-tls.md`. Kein Remote Connect, kein Zertifikatsmanager, kein Landing-Change, `dashboard_reachable: false` ehrlich.
- [x] T0920 Install media Slice 13.1: Public download/install path — `make install-media` Alias, `make release-install-media-stage`, `GET /api/v1/public/install-media`, optional `/release/*` mit `VYNTRIO_RELEASE_STAGING_DIR`, `/download` live status UI, `docs/24_INSTALL_MEDIA.md`, `docs/ops/install-media-audit.md`. Updated baseline: dual-mode BIOS+UEFI (`vyntrio-install-media.img`); BIOS-only incomplete/refused at stage. Ehrlich: kein Produktions-CDN; Secure Boot unsigned; Runtime-Boot nicht auf jedem Host bewiesen.
- [x] T0921 Install media Slice 13.2: Guided install-media creator — `/download` `PublicInstallMediaCreatorGuide` (USB + VM, checksum, warnings), `scripts/write-install-media-usb.sh`, `scripts/run-install-media-vm.sh`, `make test-write-install-media-usb`, Doku/Audit-Updates. Ehrlich: kein Browser-Writer, destruktives USB nur per CLI mit Bestätigung.
- [x] T0922 Install media Slice 13.3: Cross-platform writer CLI — `vyntrio-write-media` (`cmd/write-media`, `internal/platform/writemedia`), `make build-write-media`, `make package-write-media`, `make test-write-media`, `/download` `PublicInstallMediaWriterSection`, `docs/ops/install-media-writer.md`. Ehrlich: CLI only, kein natives GUI, kein Produktions-CDN für Writer-Binaries.
- [x] T0923 Install media Slice 13.4: Writer-Downloads pro Plattform — `writer.artifacts[]` in `GET /api/v1/public/install-media` (reale Größe/SHA-256 aus Staging), `/release/writer/{filename}` Allowlist-Downloads, `make package-write-media` staged nach `distro/release/staging/writer/`, `/download` Plattform-Verfügbarkeit (Windows .exe, macOS/Linux Binaries) mit ehrlichem "not staged"-Zustand; `docs/ops/truenas-parity-audit-2026-07-17.md`. Ehrlich: unsignierte Binaries, kein .dmg (kein macOS-Build-Host), kein CDN.
- [x] T0924 Storage Slice 12.3: Declared pool / dataset / share foundation — `POST /api/v1/storage/pools` (eligible disks, confirm, no format), dataset + share plans, Storage UI mutation flow, `state_dir/storage/pools.json`; `docs/05_STORAGE.md`, parity audit update. Ehrlich: kein Disk-Format, kein SMB/NFS.
